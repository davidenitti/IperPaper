...
\item Some equations, derivation steps, and terms are blue. For example, hover over \iperpaper{tutorial_annotation_example}{this annotated phrase} for a short explanation (AI generated), or click it to open the full explanation in the side panel (AI generated).
...
\]
Here \(\iperpaper{prf_symbol}{\PRF}\) means \emph{\iperpaper{prf_concept}{pseudorandom function}}. For a fixed key it is deterministic, but to an observer without the key its outputs are intended to be computationally indistinguishable from random values. Treating the $r_{t,i}$ values as independent $\iperpaper{uniform_symbol}{\U}(0,1)$ variables is the ideal statistical model, not a claim that the implementation produces literal randomness.
...
seed. That seed initializes the \iperpaper{pcg64_name}{PCG-64} pseudorandom generator:
\[
  \operatorname{seed}_t
  =A(w_{t-m},\ldots,w_{t-1},\Key),
  \qquad
  (r_{t,1},\ldots,r_{t,V})
  =\iperpaper{pcg64_symbol}{\operatorname{PCG64}}(\operatorname{seed}_t).
\]
...
Li et al. use $m=5$ and NumPy's \iperpaper{pcg64_name}{PCG-64} generator in this implementation. The
independent $\iperpaper{uniform_symbol}{\U}(0,1)$ behavior used in the analysis is an idealization of these
...
The ideal model treats each $r_{t,i}$ as continuous \iperpaper{uniform_name}{Uniform}$(0,1)$. Exact endpoints have probability zero. Concrete floating-point RNG APIs may include an endpoint---for example, NumPy returns values in $[0,1)$---so implementations applying the logarithmic transforms should explicitly avoid exact endpoints.
\end{leveltwo}
...
\begin{aligned}
  ...
  =\iperpaper{argmin_symbol}{\argmin}_{i:p_{t,i}>0}\iperpaper{race_time}{X_{t,i}}.
\end{aligned}
...
If $r_{t,i}\sim\iperpaper{uniform_symbol}{\U}(0,1)$, then
...
$Y\sim\iperpaper{exp_symbol}{\Exp}(\text{rate}=\lambda)$, its density and survival function are
\[
...
$\iperpaper{exp_symbol}{\Exp}(\text{rate}=1)$. For $x<0$, both survival probabilities equal $1$
because $Y_{t,i}\ge0$. Hence
$Y_{t,i}=-\log r_{t,i}\sim\iperpaper{exp_symbol}{\Exp}(\text{rate}=1)$.
\end{samepage}
\end{leveltwo}

Since $-\log r_{t,i}\sim\iperpaper{exp_symbol}{\Exp}(\text{rate}=1)$, the definition $X_{t,i}=(-\log r_{t,i})/p_{t,i}$, given immediately after Equation~\eqref{eq:log-min-forms}, implies that
\[
...
Let $E=-\log r_{t,i}$. Because $r_{t,i}\sim\iperpaper{uniform_symbol}{\U}(0,1)$, $E\sim\iperpaper{exp_symbol}{\Exp}(1)$. For
\[
...
&=e^{-p_{t,i}x}.
\end{align*}
That is exactly the survival function of an exponential random variable with rate $p_{t,i}$. Thus $X_{t,i}\sim\iperpaper{exp_symbol}{\Exp}(p_{t,i})$. Larger $p_{t,i}$ means a distribution more concentrated near zero, which makes that candidate more likely to attain the minimum.
\end{leveltwo}


From Equation~\eqref{eq:log-min-forms}, $w_t=\iperpaper{argmin_symbol}{\argmin}_{i:p_{t,i}>0}X_{t,i}$, so $w_t$ is the candidate with the smallest value among independent exponential random variables with rates $p_{t,1},\ldots,p_{t,V}$.
...
\]
This identity is commonly called an \iperpaper{exponential_race_concept}{\emph{exponential race}}. It proves Equation~\eqref{eq:unbiased-summary} and is the mathematical reason the watermark preserves the model's \iperpaper{categorical_name}{categorical} distribution exactly while still coupling the output to secret randomness.
...
For $X_{t,i}\sim\iperpaper{exp_symbol}{\Exp}(p_{t,i})$,
\[
...
\end{enumerate}
\end{samepage}
The term $\log p_{t,i}$ gives more probable candidates a larger baseline score, while $G_{t,i}$ randomizes the winner. The remarkable fact is that this competition is an exact \iperpaper{categorical_name}{categorical} sampler:
\[
  \Pp\!\left(i=\iperpaper{argmax_symbol}{\argmax}_j\{\log p_{t,j}+G_{t,j}\}\right)=p_{t,i}.
\]
...
\begin{align*}
\iperpaper{argmin_symbol}{\argmin}_i\frac{-\log r_{t,i}}{p_{t,i}}
&=\iperpaper{argmax_symbol}{\argmax}_i\left[-\log\left(\frac{-\log r_{t,i}}{p_{t,i}}\right)\right]\\
&=\iperpaper{argmax_symbol}{\argmax}_i\left(\log p_{t,i}-\log(-\log r_{t,i})\right)\\
&=\iperpaper{argmax_symbol}{\argmax}_i\bigl(\log p_{t,i}+G_{t,i}\bigr).
\end{align*}
...
If $r_{t,i}\sim\iperpaper{uniform_symbol}{\U}(0,1)$, then for any $g\in\mathbb R$,
\begin{align*}
\Pp(G_{t,i}\le g)
...
&=e^{-e^{-g}}.
\end{align*}
This is the \iperpaper{cdf_name}{CDF} of a standard Gumbel random variable. Thus the two appearances of $\log$ in $G_{t,i}=-\log(-\log r_{t,i})$ are not a typographical error; they are exactly the transform that converts a uniform variable into a standard Gumbel variable.
\end{leveltwo}

% =====================================================================
\section{Regenerating secret values from observed text}

A text by itself does \textbf{not} reveal the secret pseudorandom numbers. To verify the watermark one needs the correct secret key and the exact \iperpaper{prf_name}{PRF}/tokenization conventions. With those, the verifier does not ``extract'' the values from the text; it \textbf{recomputes} them.

For each eligible position $t$ in the observed token sequence $w_1,\ldots,w_T$, the verifier computes
\[
  r_{t,w_t}=\iperpaper{prf_symbol}{\PRF}_{\Key}(\text{prescribed context},w_t).
\]
...
\item evaluate the keyed \iperpaper{prf_name}{PRF} on that context and the observed token $w_t$;
...
A verifier generally needs the same secret key, tokenizer and token IDs, context-window rule, \iperpaper{prf_name}{PRF} or keyed hash, domain-separation/salting convention, and hash-to-$(0,1)$ conversion used by the generator. If the scheme masks repeated contexts or uses other bookkeeping, that convention must also match.

Finite-context seeding has one useful synchronization property: after a local insertion or deletion, the \iperpaper{prf_name}{PRF} inputs may be wrong for several subsequent positions, but once the edit leaves the context window, later contexts can match again. That does not make the scheme fully edit-robust, but it explains why small local edits need not destroy all later evidence.
...
$\iperpaper{uniform_symbol}{\U}(0,1)$ random variables and the watermarked generator selects
\[
  w_t=\iperpaper{argmax_symbol}{\argmax}_{i\in\mathcal S_t} r_{t,i}^{1/p_{t,i}}.
\]
...
\]
Because $r_{t,j}\sim\iperpaper{uniform_symbol}{\U}(0,1)$ and $0<u^{p_{t,j}/p_{t,i}}<1$, each competing
candidate satisfies
\[
...
\end{aligned}
\]
the \iperpaper{cdf_name}{CDF} of $\iperpaper{beta_conditional_cdf}{\Beta(1/p_{t,i},1)}$.
\end{leveltwo}
...
\]
so the alternative \iperpaper{cdf_name}{CDF} lies below the \iperpaper{uniform_name}{Uniform}$(0,1)$ \iperpaper{cdf_name}{CDF}. Equivalently, $r_{t,w_t}$ is stochastically larger under $H_1$ \cite{li2025}.
...
\midrule
Human / unwatermarked LLM & $\iperpaper{uniform_symbol}{\U}(0,1)$ & $\E[r_{t,w_t}]=0.5$ \\
Keyed Gumbel-max LLM & \iperpaper{cdf_name}{CDF} $\sum_i p_{t,i}x^{1/p_{t,i}}$ & $\E[r_{t,w_t}]>0.5$ unless the next-token distribution is singular \\
\bottomrule
\end{tabularx}
\end{center}
...
\[
  Z_t\sim\iperpaper{exp_symbol}{\Exp}(\text{rate}=1),
  \qquad
  \E_0[Z_t]=1,
  \qquad
  \Var_0(Z_t)=1.
...
\[
  w_t=\iperpaper{argmax_symbol}{\argmax}_i r_{t,i}^{1/p_{t,i}}.
\]
...
\item A verifier with the key regenerates the selected value $r_{t,w_t}$ at every scorable position. Under key-independent text, $r_{t,w_t}\sim\iperpaper{uniform_symbol}{\U}(0,1)$; under watermarking it is biased toward 1.
