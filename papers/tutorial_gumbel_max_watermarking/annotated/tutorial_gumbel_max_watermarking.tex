...
\end{align*}
This distinction is important: the detector is not a generic ``human versus LLM'' classifier. Human-written text and output from an unwatermarked LLM ordinarily fall under the same null model when their token choices are independent of the tested keyed pseudorandom field. Lack of access to the key is normally sufficient in the ideal model, but it does not by itself rule out correlations caused by copied or watermarked context. The verifier is testing for a specific secret correlation \cite{li2025}.

The tutorial develops the original Gumbel-max generation and detection mechanism from first principles, including the exponential-race argument, how the secret pseudorandom values are regenerated from a text, and the exact null distribution of the original score under the ideal independent-effective-position model. It concludes with later results on edits, repeated pseudorandom inputs, diversity, and sequential detection \cite{li2025,fu2024,liedits2025,cai2025collision,huang2026,luo2026}.
...
\]
Here \(\PRF\) means \emph{pseudorandom function}. For a fixed key it is deterministic, but to an observer without the key its outputs are intended to be computationally indistinguishable from random values. This is the ideal cryptographic abstraction. The PCG-64 construction described below is instead a reproducible experimental pseudorandom implementation, not a cryptographic PRF. Treating the $r_{t,i}$ values as independent $\U(0,1)$ variables is the ideal statistical model, not a claim that the implementation produces literal randomness.
The generator and verifier must use exactly the same context rule and token encoding \cite{li2025}.

Appendix D.1 of Li et al. specifies the following experimental construction. It concatenates
...
Generation must compare candidates, so it conceptually needs the whole pseudorandom vector. Detection already observes the chosen index $w_t$. The original statistic depends only on $r_{t,w_t}$, so unused coordinates carry no additional information for that test. With a random-access keyed function this can reduce computation; with a stream-based seeded RNG the implementation may still generate or advance through many coordinates.
\end{leveltwo}
...
\begin{align*}
\iperpaper{null_hypothesis}{H_0} &: \text{the observed text is independent of this secret watermark randomness},\\
...
\begin{equation}
  \iperpaper{null_pivot_distribution}{r_{t,w_t}\sim\U(0,1)}.
  \label{eq:nullr}
\end{equation}
Thus
\[
...
\]
This applies both to human text and to an ordinary unwatermarked LLM, provided the observed token choice is independent of the tested keyed pseudorandom field. The test is therefore a \emph{watermark-presence test}, not a generic AI-writing test \cite{li2025}.

Under watermarking, the selected value $r_{t,w_t}$ is biased toward 1. Conditional on the next-token distribution $\mathbf p_t$,
\begin{equation}
  \iperpaper{alternative_pivot_cdf}{\Pp_1(r_{t,w_t}\le x\mid\mathbf p_t)
...
Li et al. further show that Aaronson's score, while valid, is not generally optimal under their detection-efficiency criterion, and derive more efficient detection rules for specified classes of next-token distributions \cite{li2025}.
...
For this chosen-coordinate detector, the verifier does not need the original prompt, hidden system prompt, temperature, or the model's exact logits in order to calibrate the null distribution. Those quantities influence the alternative distribution and therefore the detection power, but under the ideal null the selected value $r_{t,w_t}$ is Uniform$(0,1)$ without knowing $\mathbf p_t$. This model-free null calibration is why the selected value is called a \iperpaper{pivot}{\textbf{pivot}} \cite{li2025}.
...
the model's inference/logits are deterministic and the model, prompt, decoding configuration, tokenization, and key are all fixed,
...
\cite{liedits2025}. Repeated local contexts can cause reuse or collisions in the keyed
pseudorandom mechanism, introducing dependence absent from the idealized analysis
\cite{cai2025collision}. Separately, exposure to watermarked input can induce watermark
mimicry in subsequent human or unwatermarked-model responses, motivating input-repetition
masking and longer seeding contexts \cite{khachaturov2025}. Finally, conventional
...
anytime-valid and online detection procedures that remain calibrated under such
...
\item Aaronson's original detector sums $-\log(1-r_{t,w_t})$; later statistical work treats $r_{t,w_t}$ as a pivot and studies better ways to aggregate the evidence.
\end{enumerate}
