...
```text
paper.tex / TeX project / PDF / paper URL
                    │
                    ▼
              AI agent
        - recovers exact TeX source when possible
        - understands notation in paper context
        - inserts \iperpaper{ID}{...} semantic targets
        - writes semantic annotations.json
        - leaves \eqref / figure \ref / \cite links native
                    │
                    ▼
       annotated TeX + annotations.json
                    │
                    ▼
            iperpaper build
        - compile paper with LaTeX
        - validate annotation targets:
          every PDF iperpaper:ID has metadata,
          and every metadata ID has a real PDF rectangle
        - discover native equation/figure/table/citation links
        - generate their tooltip metadata from
          .aux + equation/figure source + bibliography entries
        - reuse or refresh citation metadata by stable citation key
        - extract page-relative target rectangles
        - compile explanation math
          with the paper preamble
        - crop/convert math fragments to SVG
        - crop referenced figure artwork to PNG
        - compile the PDF in a temporary workspace and keep its bytes in memory
        - optionally write a standalone PDF with `--pdf-output`
        - build PDF-backed HTML, native HTML, or both
                    │
          ┌─────────┴──────────┐
          ▼                    ▼
      paper.html       paper.native.html
        PDF.js          Pandoc + MathJax
      exact layout      responsive text
          └─────────┬──────────┘
                    ▼
...
7. generates tooltip metadata and overlays those native rectangles without changing their color or destination.

Generated equation-reference metadata uses the extracted formula as its label, figure references use `Figure N`, table references use `Table N`, and bibliography references use their rendered `[N]` label. Figure- and table-reference hover cards show the rendered artwork together with the source caption, with the reference label in bold. Their display width defaults to 80% of the printed artwork width and is controlled by the shared `FIGURE_TOOLTIP_SCALE` setting in `iperpaper.py`; `FIGURE_PREVIEW_DPI` controls the embedded PNG resolution. Each number in a multi-citation keeps its own native PDF link and receives the tooltip for that bibliography entry. Repeated links to the same label or citation key share generated metadata. Clicking a generated tooltip target follows the underlying native PDF link.

When bibliography lookup is enabled, explicit direct PDF URLs and arXiv PDFs
take priority, followed by Crossref full-text links that return actual PDF
bytes, OpenAlex locations explicitly marked open access, and recognized
open-access PDFs reported by Semantic Scholar. A trusted DOI selects a
service's DOI lookup when available; otherwise Crossref is queried with
separate title and author parameters, and OpenAlex and Semantic Scholar are
searched by title and authors. Remote results must also match the available
trusted title and authors, including results retrieved by DOI. If one matching
OpenAlex or Semantic Scholar record has no usable PDF, IperPaper continues
through the other matching candidates. OpenAlex records marked `closed` or
`BRONZE` are left unlinked because their PDF may still require a subscription.
Repository copies that are not exposed by these services must be supplied as
an explicit bibliography URL.
...
identifier, a Crossref full-text URL verified by its PDF bytes, a direct open
PDF found through a high-confidence OpenAlex match, a recognized open-access
PDF reported by Semantic Scholar, and finally the first explicit HTTP(S) URL
as a fallback. An explicit landing-page URL can therefore be replaced by a
verified open PDF. Crossref uses a DOI when supplied; otherwise it queries the
title and authors separately. OpenAlex and Semantic Scholar likewise use the
DOI when available and also search by title and authors. Every remote candidate
must match the available trusted title and authors. IperPaper keeps scanning
matching OpenAlex and Semantic Scholar candidates when an earlier result has no
usable PDF. Citations without a usable URL retain their normal reference
behavior. The `--no-citation-link-lookup` option disables external citation
service queries; existing cached metadata is still reused.
...
- The PDF-backed reader uses the PDF compiled locally from the annotated source. Mapping annotations onto a separately supplied publisher/original PDF requires an additional PDF-registration step.
...
- The PDF-backed reader loads pinned PDF.js assets from jsDelivr; native HTML loads pinned MathJax assets from jsDelivr. Either therefore needs network access for its renderer unless those assets are hosted locally.
...
- Very unusual multi-line or transformed hyperlink contents can require special rectangle handling.
